...
class MyProvider(Provider):
    ...
    async def _get_chats_object(self, config: Config) -> Chats:
        return Chats(
            generator=ChatGenerator(config=config.ai.gemini), gemini=config.ai.gemini
        )
    ...
    @provide(scope=Scope.APP)
    async def _get_chat_service(self, chats: Chats, config: Config) -> ChatService:
